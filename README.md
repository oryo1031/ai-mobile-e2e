...
「開いていないのに次のステップへ進む」と、失敗の原因がまったく別の場所に
見えてしまうため。

### テスト間で状態を残すか

`e2e.config.yaml` の `no_reset` で決める。既定は `false`。

| 値 | 挙動 |
|---|---|
| `false`(既定) | テストごとにアプリの状態を消す。毎回ログインからやり直すので遅いが、テスト同士が影響しない |
| `true` | 状態が残る。速いが、**セットアップが「既にその状態」を扱えないと 2 件目以降が壊れる** |

`true` にする場合は、セットアップが冒頭で今の状態を確かめて、
既に満たしていればすぐ返す形になっている必要がある。この規約は
`tests/setup/__init__.py` とプロンプトに書いてある。
...
`e2e resume` は**完了していない最初の工程**から再開する。
成果物が揃っていて検証を通る工程は飛ばすので、同じものを作り直さない。

```
[5/8] テストコードの生成 (codegen)
    成果物が既に揃っており検証も通ったため、この工程は飛ばします。
```

プロンプトを直したなどの理由で**あえて作り直したい**場合は `--redo` を使う。
指定した工程とそれ以降が対象になる。

```bash
uv run e2e resume --redo codegen
```

### 実行だけを繰り返す

端末側の確認を繰り返す間は、ワークフローを通さず pytest を直接叩く方が速い。
Copilot を呼ばないので、端末やアプリ側の問題を切り分けるのに向く。

```bash
uv run pytest tests/e2e \
  --platform=android \
  --evidence-dir=artifacts/<run-id>/evidence \
  --junitxml=artifacts/<run-id>/junit.xml -v
```

`execute` 工程が実行しているのと同じコマンドで、証跡も同じ場所に残る。
