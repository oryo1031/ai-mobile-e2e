...
**Copilot CLI の有無で自動的に決まる。** どちらになったかは `e2e run` の
出力の「実行方式」に表示される。`--mode cli` / `--mode manual` で明示もできる。

| | CLI モード | 手動モード |
|---|---|---|
| 条件 | `copilot` コマンドがある | 無い(VS Code の拡張のみ) |
| AI 工程 | `e2e run` が自動で呼ぶ | プロンプトを書き出して停止。人がチャットに貼る |
...
1. `e2e run`(初回)または `e2e resume`(2 回目以降)を実行する
2. 表示されたプロンプトのファイルを開く
3. VS Code の Copilot Chat で、表示されたエージェントを選んで貼り付けて実行する
4. 生成物が保存されたことを確認して 1 に戻る
