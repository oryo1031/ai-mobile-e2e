...
**設計書に正確な URL が書かれていないことを前提とする。** URL は運用上
`testdata/deeplinks.yaml` に人が手で書くことになっている。

したがって、URL が読み取れなくても `open_questions` に挙げなくてよい。
どこへ遷移する導線があるかだけを `flows` に残せばよい。

設計書に URL が明記されている場合に限り、`action: open_deeplink` の
ステップとして `value` に入れてよい。**書かれていない URL を推測で
作らない。** スキームを勝手に決めると、実行時に「開かない」形で失敗し、
URL が違うのかアプリの不具合なのかが分からなくなる。
